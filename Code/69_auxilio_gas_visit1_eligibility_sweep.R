...
out_dir <- here("data", "powerIV", "regressions", "auxilio_gas_visit1_eligibility_sweep")
...
households <- read_parquet(
  ...
  mutate(
    ...
    eligible_vd5002_or_bpc = (!is.na(vd5002_pc) & !is.na(half_min_wage) & vd5002_pc <= half_min_wage) | has_bpc_visit1 == 1,
...
household_samples <- tibble::tribble(
  ...
  "eligible_vd5002_or_bpc_households", "eligible_vd5002_or_bpc == TRUE",
...
women <- read_parquet(
  ...
  mutate(
    ...
    eligible_vd5002_or_bpc = (!is.na(visit1_vd5002_pc) & !is.na(visit1_half_min_wage) & visit1_vd5002_pc <= visit1_half_min_wage) | visit1_has_bpc == 1,
...
sample_defs <- tibble::tribble(
  ...
  "eligible_vd5002_or_bpc", "eligible_vd5002_or_bpc == TRUE",
  "eligible_vd5008_bolsa_or_bpc", "eligible_vd5008_bolsa_or_bpc == TRUE",
  "eligible_vd5008_or_bpc_kids", "eligible_vd5008_or_bpc == TRUE & n_children_0_14 > 0",
  "eligible_vd5008_or_bpc_wood", "eligible_vd5008_or_bpc == TRUE & visit1_cooking_wood_charcoal == 1",
...
sample_summary <- women %>%
  mutate(
    ...
    eligible_vd5002_or_bpc_flag = as.integer(coalesce(eligible_vd5002_or_bpc, FALSE)),
    eligible_vd5008_bolsa_or_bpc_flag = as.integer(coalesce(eligible_vd5008_bolsa_or_bpc, FALSE)),
    eligible_vd5008_or_bpc_kids_flag = as.integer(coalesce(eligible_vd5008_or_bpc, FALSE) & coalesce(n_children_0_14, 0) > 0),
    eligible_vd5008_or_bpc_wood_flag = as.integer(coalesce(eligible_vd5008_or_bpc, FALSE) & coalesce(visit1_cooking_wood_charcoal, 0) == 1),
  ...
  summarise(
    ...
    eligible_vd5002_or_bpc = sum(eligible_vd5002_or_bpc_flag, na.rm = TRUE),
    eligible_vd5008_bolsa_or_bpc = sum(eligible_vd5008_bolsa_or_bpc_flag, na.rm = TRUE),
    eligible_vd5008_or_bpc_kids = sum(eligible_vd5008_or_bpc_kids_flag, na.rm = TRUE),
    eligible_vd5008_or_bpc_wood = sum(eligible_vd5008_or_bpc_wood_flag, na.rm = TRUE),
    share_has_bpc_visit1 = mean(coalesce(visit1_has_bpc, 0) == 1, na.rm = TRUE),
    share_has_bolsa_visit1 = mean(coalesce(visit1_has_bolsa, 0) == 1, na.rm = TRUE),
    share_has_other_prog_visit1 = mean(coalesce(visit1_has_other_prog, 0) == 1, na.rm = TRUE),
    ...
    share_eligible_vd5008_or_bpc = mean(eligible_vd5008_or_bpc_flag, na.rm = TRUE),
    share_eligible_vd5008_bolsa_or_bpc = mean(eligible_vd5008_bolsa_or_bpc_flag, na.rm = TRUE)
  )
