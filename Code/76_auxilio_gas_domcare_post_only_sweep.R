...
out_dir <- here("data", "powerIV", "regressions", "auxilio_gas_domcare_post_only_sweep")
...
treatment_panel <- visit1_q %>%
  ...
  filter(year > 2022 | (year == 2022 & quarter >= 3))

treatment_panel <- treatment_panel %>%
  select(estrato4, year, quarter, year_quarter, families_per_weighted_household)

treatments <- tibble::tribble(
  ~treatment, ~design, ~design_label,
  "families_per_weighted_household", "post_only_level", "Post-only quarterly level"
)
...
write.table(
  results,
  file.path(out_dir, "domcare_post_only_results.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.table(
  paper_table,
  file.path(out_dir, "domcare_post_only_paper_table.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.table(
  sample_summary,
  file.path(out_dir, "domcare_post_only_sample_summary.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
