...
out_dir <- here("data", "powerIV", "regressions", "auxilio_gas_domcare_pooled_sweep")
...
treatments <- tibble::tribble(
  ...
  "families_per_weighted_household", "pooled_level", "Pooled quarterly level"
)
...
write.table(
  results,
  file.path(out_dir, "domcare_pooled_results.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.table(
  paper_table,
  file.path(out_dir, "domcare_pooled_paper_table.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.table(
  sample_summary,
  file.path(out_dir, "domcare_pooled_sample_summary.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
