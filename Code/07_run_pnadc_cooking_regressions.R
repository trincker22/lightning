...
panel_path <- file.path(root_dir, "data", "powerIV", "pnadc_power", "panels", "pnadc_estrato4_quarter_with_power.parquet")
split_panel_path <- file.path(root_dir, "data", "powerIV", "pnadc", "pnadc_visit1_estrato_quarter_urban_rural_2016_2024.parquet")
out_dir <- file.path(root_dir, "data", "powerIV", "regressions", "cooking")
fig_dir <- file.path(root_dir, "Figures", "powerIV", "cooking")
tab_dir <- file.path(out_dir, "tables")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(tab_dir, recursive = TRUE, showWarnings = FALSE)

results_path <- file.path(tab_dir, "pnadc_power_regressions.csv")
first_stage_path <- file.path(tab_dir, "pnadc_power_first_stage_summary.csv")
clean_path <- file.path(tab_dir, "pnadc_cooking_fuel_clean_results.csv")
...
panel <- panel %>%
  select(-matches("_l4$")) %>%
  left_join(power_only, by = c("estrato4", "year", "quarter", "quarter_start", "lightning_quarterly_total", "raw_quarterly_outage_hrs_per_1000", "dec_quarterly"))
...
all_raw <- panel %>% filter(year %in% 2017:2019, !is.na(lightning_quarterly_total), !is.na(raw_quarterly_outage_hrs_per_1000))
all_raw_l4 <- panel %>% filter(year %in% 2017:2019, !is.na(lightning_quarterly_total_l4), !is.na(raw_quarterly_outage_hrs_per_1000_l4))
rural_raw <- split_panel %>% filter(urban_rural == "rural", year %in% 2017:2019, !is.na(lightning_quarterly_total), !is.na(raw_quarterly_outage_hrs_per_1000))
rural_raw_l4 <- split_panel %>% filter(urban_rural == "rural", year %in% 2017:2019, !is.na(lightning_quarterly_total_l4), !is.na(raw_quarterly_outage_hrs_per_1000_l4))
urban_raw <- split_panel %>% filter(urban_rural == "urban", year %in% 2017:2019, !is.na(lightning_quarterly_total), !is.na(raw_quarterly_outage_hrs_per_1000))

all_dec <- panel %>% filter(year %in% 2016:2019, !is.na(lightning_quarterly_total), !is.na(dec_quarterly))
all_dec_l4 <- panel %>% filter(year %in% 2016:2019, !is.na(lightning_quarterly_total_l4), !is.na(dec_quarterly_l4))
rural_dec <- split_panel %>% filter(urban_rural == "rural", year %in% 2016:2019, !is.na(lightning_quarterly_total), !is.na(dec_quarterly))
rural_dec_l4 <- split_panel %>% filter(urban_rural == "rural", year %in% 2016:2019, !is.na(lightning_quarterly_total_l4), !is.na(dec_quarterly_l4))
urban_dec <- split_panel %>% filter(urban_rural == "urban", year %in% 2016:2019, !is.na(lightning_quarterly_total), !is.na(dec_quarterly))
...
plot_coefficients(
  results_tbl %>% filter(outcome == "share_cooking_wood_charcoal", model_family %in% c("reduced_form", "iv")),
  "Cooking with wood/charcoal",
  file.path(fig_dir, "figure_1_cooking_wood_charcoal.png")
)

plot_coefficients(
  results_tbl %>% filter(outcome == "share_cooking_electricity", model_family %in% c("reduced_form", "iv")),
  "Cooking with electricity",
  file.path(fig_dir, "figure_2_cooking_electricity.png")
)
...
cat(clean_path, "\n")
cat(file.path(fig_dir, "figure_1_cooking_wood_charcoal.png"), "\n")
cat(file.path(fig_dir, "figure_2_cooking_electricity.png"), "\n")
