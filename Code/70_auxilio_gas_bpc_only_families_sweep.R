...
out_dir <- here("data", "powerIV", "regressions", "auxilio_gas_bpc_only_families_sweep")
...
household_samples <- tibble::tribble(
  ~sample, ~filter_expr,
  "bpc_only_households", "bpc_only == TRUE"
)
...
write.table(
  household_results,
  file.path(out_dir, "bpc_only_household_results.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
sample_defs <- tibble::tribble(
  ~sample, ~filter_expr,
  "bpc_only", "bpc_only == TRUE",
  "bpc_only_kids", "bpc_only == TRUE & n_children_0_14 > 0",
  "bpc_only_wood", "bpc_only == TRUE & visit1_cooking_wood_charcoal == 1"
)
...
write.table(
  cross_results,
  file.path(out_dir, "bpc_only_cross_results.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.table(
  fe_results,
  file.path(out_dir, "bpc_only_fe_results.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
write.table(
  confirmed_hits,
  file.path(out_dir, "bpc_only_confirmed_hits.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
sample_summary <- women %>%
  mutate(
    bpc_only_flag = as.integer(coalesce(bpc_only, FALSE)),
    bpc_only_kids_flag = as.integer(coalesce(bpc_only, FALSE) & coalesce(n_children_0_14, 0) > 0),
    bpc_only_wood_flag = as.integer(coalesce(bpc_only, FALSE) & coalesce(visit1_cooking_wood_charcoal, 0) == 1),
  ...
  summarise(
    ...
    bpc_only = sum(bpc_only_flag, na.rm = TRUE),
    bpc_only_kids = sum(bpc_only_kids_flag, na.rm = TRUE),
    bpc_only_wood = sum(bpc_only_wood_flag, na.rm = TRUE),
    share_bpc_only = mean(bpc_only_flag, na.rm = TRUE),
...
write.table(
  sample_summary,
  file.path(out_dir, "bpc_only_sample_summary.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
